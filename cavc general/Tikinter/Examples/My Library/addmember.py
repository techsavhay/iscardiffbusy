...
class AddMember(Toplevel):
    ...
    def addMember(self):
        ...
        if (name and phone !=""):
            try:
                query="INSERT INTO dbo.members (member_name,member_phone) VALUES(?,?)"
                vals = (name,phone)
                print (vals)
                cur.execute(query,vals)
                con.commit()
                messagebox.showinfo("Success","Successfully added to Library database",icon='info')

            except pyodbc.Error as ex:
                #messagebox.showerror("Error","Cant add to Library database",icon='warning')
                print('Database Connection Issue', ex)
        else:
